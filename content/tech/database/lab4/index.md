---
title: "CMU15-445|Project4|并发控制"
description: 单机数据库的并发控制，通过2PL实现不同隔离级别
date: 2023-06-01T21:51:34+08:00
image: blogimage-20230710185804351.png
math: 
license: 
hidden: false
comments: true
...
- 相比更高一级的隔离级别，我缺少了什么隔离性？
- 相比低一级的，我又提供了什么功能？

举个例子：Read Commited隔离级别， 相比Read Uncommited(读未提交)，每次需要读到的必须是**已经提交过**的数据，而不是已修改而未提交的数据。所以RC相比RU，需要给每个行加上读锁来实现这个功能（因为共享锁与排他锁冲突，如果能加读锁，该行**一定不是还处于事务进行中的修改**）。

那么再看更高一级呢？RC相比可重复读， 不需要在同一次事务的多次读中获得相同数据，所以我们可以在每次读取后立刻释放读锁，而RR相比RC，需要一直持有读锁。
